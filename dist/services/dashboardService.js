...
exports.getCreatorDashboard = exports.getEventeeDashboard = void 0;
...
const getEventeeDashboard = async (userId) => {
    const now = new Date();
    /* ---------------- USER ---------------- */
    const user = await userModel_1.User.findById(userId).select("name");
    if (!user) {
        throw new Error("User not found");
    }
    /* ---------------- TICKETS ---------------- */
    const tickets = await (0, ticketService_1.getEventeeTickets)({ eventeeId: userId });
    ...
    const paymentHistory = payments.map(payment => ({
        ...
        eventDate: payment.eventId?.startTime,
    }));
    /* ---------------- FINAL RESPONSE ---------------- */
    return {
        userName: user.name,
...
exports.getEventeeDashboard = getEventeeDashboard;
const getCreatorDashboard = async (creatorId) => {
    const now = new Date();
    /* ---------------- USER ---------------- */
    const user = await userModel_1.User.findById(creatorId).select("name");
    if (!user)
        throw new Error("User not found");
    /* ---------------- EVENTS ---------------- */
    const events = await eventModel_1.Event.find({ creatorId: new mongoose_1.Types.ObjectId(creatorId) }, {
        title: 1,
        startTime: 1,
        endTime: 1,
        totalTickets: 1,
    ...
    /* ---------------- AGGREGATION ---------------- */
    const ticketStats = await ticketModel_1.Ticket.aggregate([
    ...
    const ticketMap = new Map(ticketStats.map(stat => [stat._id.toString(), stat]));
    /* ---------------- STATS ---------------- */
    const stats = {
    ...
    const eventsNeedingAttention = events
        ...
        const stat = ticketMap.get(event._id.toString());
        const sold = stat?.ticketsSold || 0;
        const daysLeft = (new Date(event.startTime).getTime() - now.getTime()) /
            (1000 * 60 * 60 * 24);
        ...
        .filter(e => e.daysLeft <= 20 && //show events that will happen in 20 days or less
        e.daysLeft > 0 &&
        e.capacityPercent < 0.3 //and events that have sold less than 30% 
    )
        .slice(0, 3);
    /* ---------------- RECENT ACTIVITY ---------------- */
    const recentActivityRaw = await paymentModel_1.Payment.find({
        eventId: { $in: eventIds },
        status: "SUCCESS",
    }, {
        eventId: 1,
    ...
    const recentActivity = recentActivityRaw.map(payment => {
        ...
        const buyerName = payment.userId?.name;
        return {
            eventTitle,
